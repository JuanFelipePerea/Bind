...
def event_create(request):
    ...
    if request.method == 'POST':
        ...
        selected_modules = request.POST.getlist('modules')  # list of module_type strings
        ...
        # Fetch the template with all related data needed by the service
        template = None
        if template_id:
        ...
        # Apply smart defaults when a template is selected
        if template:
            if not description:
        ...
        with transaction.atomic():
            ...
            if template:
                # allowed_modules=None means "use all template modules"
                allowed = selected_modules if selected_modules else None
                apply_template_to_event(event, template, owner=request.user, allowed_modules=allowed)
    ...
    # Pre-select template from URL param (e.g. coming from template_list)
